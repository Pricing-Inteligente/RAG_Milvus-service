...
def chat_stream(req: ChatReqStream):
    ...
    if macros:
        ...
        if not countries and req.session_id:
            ...
            # --- NUEVO: superlativos macro sin países explícitos ---
        direction = _is_macro_superlative_query(text)
        if direction and not countries and macros and macros[0] != "__ALL__":
            var = _canonicalize_macro_var(macros[0])
            cs  = _macro_default_countries()
            rows = _macro_rank(var, cs)

            if not rows:
                reason = "no hay series disponibles para esa variable en los países por defecto."
                return StreamingResponse(_sse_no_data_ex(reason, {"country": cs, "variable": var}),
                                        media_type="text/event-stream")

            best = (max if direction == "max" else min)(rows, key=lambda x: x["value"])

            def gen_rank():
                yield f"data: [MACRO] ranking → variable: {var} | países: {' | '.join(cs)} | modo: {'máximo' if direction=='max' else 'mínimo'}\n\n"

                # Prompt humano para redactar (ya usas _prompt_macro_humano)
                facts = {
                    "type": "macro_superlative",
                    "variable": var,
                    "direction": direction,     # "max" o "min"
                    "rows": rows,               # lista con valores por país
                    "best": best,               # país ganador
                }
                prompt = _prompt_macro_humano(
                    "macro_superlative",
                    facts,
                    "¿Te muestro el top 5 o lo comparo con otro indicador?"
                )
                for chunk in _stream_no_fin(prompt):
                    yield chunk
                yield "data: \n\n"

                # Línea determinista de resultado
                unit = best.get("unit") or ""
                date = best.get("date") or "-"
                yield f"data: Resultado: {best['country']} — {best['value']} {unit} (fecha: {date})\n\n"
                yield "data: [FIN]\n\n"

            return StreamingResponse(gen_rank(), media_type="text/event-stream")

    
                # --- Superlativos macro: “¿qué país tiene X más alto/bajo?” sin país explícito
        superl = _is_macro_superlative_query(text)
        if superl and not countries and macros and "__ALL__" not in macros:
            # comparamos en todos los países soportados
            countries = list(COUNTRY_ALIASES.keys())
            var = macros[0]  # primera variable mencionada (p.ej., "cpi")

            rows = macro_compare(var, countries) or []
            if not rows:
            ...
            # elegir el ganador según MAX o MIN
            key = (lambda r: float(r.get("value") or 0.0))
            best = (max(rows, key=key) if superl == "max" else min(rows, key=key))

            def gen_super():
                yield f"data: Filtros → variable: {var} | países: {' | '.join(countries)}\n\n"

                # Hechos para redacción humana
                facts = {
                    "type": "macro_compare",
                    "variable": var,
                    "countries": countries,
